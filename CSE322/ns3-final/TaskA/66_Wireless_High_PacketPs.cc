...
void superComputation(int packetPs, string fileName)
{
    cout << endl << endl << "---" << endl;
    cout << "packetPs: " << packetPs << endl; 
    ...
    string DATA_RATE = to_string(PACKET_SIZE_KB * 8 * packetPs) + "kbps";
    ...
    // "PacketP/s, Throughput(kbps), Delay (ns), Drop Ratio, Delivery Ratio"
    outfile << packetPs << "," << avgTp << "," << avgDelay.GetNanoSeconds() << "," << dropRatio << "," << deliveryRatio << endl;
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    string fileName = "66_Wireless_High_PacketPs";

    outfile.open(fileName + ".dat", ios_base::app);
    outfile << "Packet Per Sec,Throughput (kbps),Delay (ns),Drop Ratio,Delivery Ratio" << endl;

    vector<int> nPacket = {50, 100, 150, 200, 250, 300, 350, 400, 450, 500};
    for (size_t i=0; i < nPacket.size(); i++)
    {
        int packetPs = nPacket[i];
        superComputation(packetPs, fileName);
    }
