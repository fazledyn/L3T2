...
void superComputation(int packetpersecond)
{
  ...
  // "PacketPs, Throughput(kbps), Delay (ns), Drop Ratio, Delivery Ratio"
  outfile << packetpersecond << "," << avgTp << "," << avgDelay.GetNanoSeconds() << "," << dropRatio << "," << deliveryRatio << endl;
...
int main(int argc, char **argv)
{
  ...
  string fileName = "66_Wireless_Low_PacketPs.dat";
  outfile.open(fileName, ios::app);
  outfile << "Packet per Second,Throughput (kbps),Delay (ns),Drop Ratio, Delivery Ratio" << endl;

  vector<int> nPacketPs = {100, 200, 300, 400, 500, 600, 700, 800, 900, 1000};
  for (size_t i=0; i < nPacketPs.size(); i++)
  {
    int ppsCount = nPacketPs[i];
    cout << "value:" << endl;
    cout << nPacketPs[i] << endl;
    superComputation(ppsCount);
  }
