...
void superComputation(double nodeSpeed)
{
  ...
  // "Speed, Throughput(kbps), Delay (ns), Drop Ratio, Delivery Ratio"
  outfile << nodeSpeed << "," << avgTp << "," << avgDelay.GetNanoSeconds() << "," << dropRatio << "," << deliveryRatio << endl;
...
int main(int argc, char **argv)
{
  ...
  string fileName = "66_Wireless_Low_Speed.dat";
  outfile.open(fileName, ios::app);
  outfile << "Speed (ms-1),Throughput (kbps),Delay (ns),Drop Ratio, Delivery Ratio" << endl;

  vector<double> nNodeSpeed = {5, 10, 15, 20, 25, 30, 35, 40, 45, 50};
  for (size_t i=0; i < nNodeSpeed.size(); i++)
  {
    double nodeSpeed = nNodeSpeed[i];
    cout << "value:" << endl;
    cout << nNodeSpeed[i] << endl;
    superComputation(nodeSpeed);
  }
