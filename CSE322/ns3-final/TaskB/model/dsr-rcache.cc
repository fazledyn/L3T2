...
namespace ns3
{
	...
	namespace dsr
	{
		...
		DsrRouteCache::RebuildBestRouteTable(Ipv4Address source)
		{
			...
			std::map<Ipv4Address, uint32_t> spEstimate;
			// @pre preceding node
			...
			{
				std::stringstream ss;
				ss << "i->first: " << i->first << std::endl;
				ss << "i->second->size: " << i->second.size() << std::endl;
				NS_LOG_UNCOND(ss.str());

				if (i->second.find(source) != i->second.end())
				{
					spEstimate[i->first] = i->second[source];
					pre[i->first] = source;
				}
				else
				{
					spEstimate[i->first] = MAXWEIGHT;
			...
			std::map<Ipv4Address, bool> nodesetDistCalculated;
			double temp = MAXWEIGHT;
			Ipv4Address tempip = Ipv4Address("255.255.255.255");
			for (uint32_t i = 0; i < m_netGraph.size(); i++)
			{
				temp = MAXWEIGHT;
				for (std::map<Ipv4Address, uint32_t>::const_iterator j = spEstimate.begin(); j != spEstimate.end(); ++j)
				{
					Ipv4Address ip = j->first;
					if (nodesetDistCalculated.find(ip) == nodesetDistCalculated.end())
					{
						if (j->second <= temp)
						{
							temp = j->second;
				...
				{
					nodesetDistCalculated[tempip] = true;
					for (std::map<Ipv4Address, uint32_t>::const_iterator k = m_netGraph[tempip].begin(); k != m_netGraph[tempip].end(); ++k)
					{
						if (nodesetDistCalculated.find(k->first) == nodesetDistCalculated.end() && spEstimate[k->first] > spEstimate[tempip] + k->second)
						{
							spEstimate[k->first] = spEstimate[tempip] + k->second;
							pre[k->first] = tempip;
						}
						/*
						 ...
						 */
						else if (spEstimate[k->first] == spEstimate[tempip] + k->second)
						{
		...
		DsrRouteCache::Print(std::ostream &os)
		{
			...
			for (std::list<DsrRouteCacheEntry>::const_iterator i =
					 m_routeEntryVector.begin();
				 i != m_routeEntryVector.end(); ++i)
			{
				i->Print(os);
			}
			os << "\n";
